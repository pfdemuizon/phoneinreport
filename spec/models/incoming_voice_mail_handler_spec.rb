...
describe IncomingVoiceMailHandler do
  before(:each) do
    email = mock('email', 
        :date => Time.now.to_s, 
        ...
        :attachments => [mock('attachment', :original_filename => 'ak02rbw2.wav', :content_type => 'audio/wav', :read => 'datadatadata')]
    )
    ...
    @mock_campaign = mock_model(Campaign, :reports => @mock_reports) 
  end
  describe "when receiving voice mails" do
    it "should pull up correct campaign" do
      ...
      IncomingVoiceMailHandler.receive(@email)
    end
